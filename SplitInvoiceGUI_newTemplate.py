...
#    nos = input('please enter the number of splits you would like to create\n')      #input number of splits you want
    ...
    df['HS CODE'] = df['HS CODE'].astype(str).astype('category')  #convert HS CODE column from int to str
    ...
    df['Sensitivity']= df['HS CODE'].apply(check_HS)    # add a columns to check if HS is safe or Sensitive
    ...
    print(df.pivot_table(index=['Sensitivity'], values=['CTN', 'QTY'], aggfunc=[np.sum]))


    sens_ct = df.loc[df['Sensitivity'] == 'Sensitive'].CTN.sum()     #sens_ct means total CARTONS of sensitivity HS CODE
    total_ct = df.CTN.sum()
    ...
    # num_unique = df.groupby('MARKS & NO')['MARKS & NO'].nunique()  # count unique FBA number, which equls to how many cartons
    # print('Total cartons of unique FBA: {} ctns'.format(len(num_unique)))

    nof = len(df.groupby('MARKS & NO')) # number of cartons
    print('Total cartons of unique FBA: {} ctns'.format(nof))

    noi = len(df.groupby('FULL DESCRICTION OF GOODS')) # number of items
    print('Total numbers of unique items: {}'.format(noi))

    noHS = len(df.groupby('HS CODE'))   #number of unique HS CODE
    print('Total numbers of unique HS CODE: {}'.format(noHS))
    l_HS = list(df.drop_duplicates('HS CODE', keep='first')['HS CODE'])      #list of unique HS CODE
    pprint(l_HS)
    ...
    nos_HS = len(df[df['HS CODE'].str.startswith('42') | df['HS CODE'].str.startswith('61') | df['HS CODE'].str.startswith('62')])
    print('Total lines of sensitive HS CODE, start with 42, 61, 62 : {}'.format(nos_HS))

    table = pd.pivot_table(df,index=['FULL DESCRICTION OF GOODS'], values=['QTY', 'CTN', 'DUTY'], aggfunc=[np.sum], margins=True, margins_name='Total')
    column_order = ['QTY', 'CTN', 'DUTY']
...
#    print('VAT total: {:.2f}\n'.format(Vat_total))
    ...
    for name, group in df.groupby('FULL DESCRICTION OF GOODS'):
    ...
    df_multiSKU = df.loc[df['MARKS & NO'].duplicated(keep=False), :]  # create dataframe for cartons that 1 FBA contains multi SKU
    df_multiSKU['MARKS & NO'].unique()
    qouf = len(df_multiSKU['MARKS & NO'].unique())         # cartons of FBA contains multi SKU

    df_singleSKU = df.drop_duplicates('MARKS & NO', keep=False) # create dataframe for cartons that 1 FBA contains 1 SKU
    ...
    for name, group in df_multiSKU.groupby('MARKS & NO'):
        if lst[counter%split_limit].CTN.sum() < split_result[counter%split_limit]:
            lst[counter%split_limit] = lst[counter%split_limit].append(group, ignore_index=False, sort=True)
        else:
            for i in range(split_limit):
                if lst[(counter + i + 1)%split_limit].CTN.sum() < split_result[(counter + i + 1)%split_limit]:
    ...
    for index, data in enumerate(lst):
        data['CT_PERC'] = round(data['CTN'] / data['CTN'].sum() * 100, 2)
        data['QTY_PERC'] = round(data['QTY'] / data['QTY'].sum() * 100, 2)
        print('\n')
        print('*' * 40)
        print('split {} : {:.0f} ctns'.format(index + 1, data.CTN.sum()))
    ...
    for data, i in zip(lst, range(len(split_result))):
        data.to_excel(writer, sheet_name = 'split_'+str(i+1) +'--'+ str(split_result[i]) + '_pcs', columns=columns, index=False)
    writer.save()
